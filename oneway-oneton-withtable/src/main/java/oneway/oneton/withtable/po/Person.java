package oneway.oneton.withtable.po;
...
//Person与Address的关系是1对N 使用关联表进行关联
...
public class Person {
    ...
    @JoinTable(name = "person_address", //指定关联表名
            joinColumns = {
                    @JoinColumn(name = "person_id", referencedColumnName = "id"), //指定关联表中用于关联Person表的外键
            },
            inverseJoinColumns = {
                    @JoinColumn(name = "address_id", referencedColumnName = "id", unique = true)  //指定关联表中用于关联Address表的外键
            })
