package oneway.ntoone.withtable.po;
...
//Person与Address的关系是N对1，使用关联表进行关联
...
public class Person {
    ...
    private int id;
    //声明Person类型关联Address，关系为N-1
    @ManyToOne(targetEntity = Address.class)
    //指定Person与Address的关联表
    @JoinTable(name = "person_address", //指定关联表名
            joinColumns = {
                    @JoinColumn(name = "person_id", referencedColumnName = "id", unique = true), //指定关联表中用于关联Person表的外键
            },
            inverseJoinColumns = {
                    @JoinColumn(name = "address_id", referencedColumnName = "id")//指定关联表中用于关联Address表的外键
            })
